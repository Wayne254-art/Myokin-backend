...
async function main() {
  for (const [sortOrder, name] of categories.entries()) await prisma.category.upsert({ where: { slug: slug(name) }, update: {}, create: { name, slug: slug(name), sortOrder, description: `Curated ${name.toLowerCase()} essentials.` } })
  ...
  for (const item of products) { const { image, ...data } = item; await prisma.product.upsert({ where: { sku: item.sku }, update: {}, create: { ...data, slug: slug(item.name), description: `${item.name} is a considered daily ritual for balanced, radiant skin.`, shortDescription: 'A high-performing essential for your daily beauty ritual.', categoryId: skincare.id, brandId: brand.id, status: 'ACTIVE', skinType: ['All skin types'], hairType: [], concerns: ['Dullness'], images: { create: { url: image, altText: item.name } } } }) }
  for (const zone of [{ name:'Nairobi Metro', county:'Nairobi', fee:250, estimatedDays:'1–2 business days' },{ name:'Kiambu & Ruiru', county:'Kiambu', fee:300, estimatedDays:'1–2 business days' },{ name:'Nationwide', county:'Other', fee:450, estimatedDays:'2–4 business days' }]) await prisma.deliveryZone.upsert({ where: { id: slug(zone.name) }, update: zone, create: { id: slug(zone.name), ...zone, freeDeliveryThreshold: 5000 } })
  const email = process.env.ADMIN_EMAIL; const password = process.env.ADMIN_PASSWORD
  if (email && password) await prisma.user.upsert({ where: { email }, update: { role: 'SUPER_ADMIN' }, create: { firstName: 'Store', lastName: 'Administrator', email, passwordHash: await bcrypt.hash(password, 12), role: 'SUPER_ADMIN', emailVerified: true } })
