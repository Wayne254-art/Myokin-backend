...
const requiredText = (label, min) => z
    .string()
    .trim()
...
const address = z.object({
    ...
    phone: z
        ...
        .pipe(z
        .string()
        .regex(/^(?:\+?254|0)[17]\d{8}$/, "Enter a valid mobile number beginning with 01 or 07, e.g. 0111 239 949.")),
...
ordersRouter.post("/", asyncHandler(async (req, res) => {
    ...
    const discount = calculateDiscount(subtotal, coupon && {
        ...
        minimumSpend: coupon.minimumSpend == null ? null : Number(coupon.minimumSpend),
    ...
    const deliveryFee = fulfilmentMethod === "PICKUP"
        ? 0
        : zone
            ? calculateDelivery(subtotal, Number(zone.fee), zone.freeDeliveryThreshold == null
                ? null
...
ordersRouter.get("/", asyncHandler(async (req, res) => res.json({
    success: true,
