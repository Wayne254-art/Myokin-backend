...
cartRouter.post('/items', asyncHandler(async (req, res) => { const input = z.object({ productId: z.string(), variantId: z.string().optional(), quantity: z.number().int().min(1).max(20).default(1) }).parse(req.body); const product = await prisma.product.findFirst({ where: { id: input.productId, isActive: true }, include: { variants: true } }); if (!product)
    throw new AppError(404, 'Product not found.'); const stock = input.variantId ? product.variants.find(v => v.id === input.variantId)?.stockQuantity : product.stockQuantity; if (stock == null || stock < input.quantity)
    throw new AppError(409, 'This product is currently out of stock.'); const cart = await getCart(req, true); const existing = await prisma.cartItem.findFirst({ where: { cartId: cart.id, productId: product.id, variantId: input.variantId ?? null } }); if (existing)
    await prisma.cartItem.update({ where: { id: existing.id }, data: { quantity: { increment: input.quantity } } });
else
    await prisma.cartItem.create({ data: { cartId: cart.id, ...input, unitPrice: input.variantId ? product.variants.find(v => v.id === input.variantId)?.price ?? product.price : product.price } }); res.status(201).json({ success: true, data: await getCart(req) }); }));
cartRouter.patch('/items/:id', asyncHandler(async (req, res) => { const { quantity } = z.object({ quantity: z.number().int().min(1).max(20) }).parse(req.body); const cart = await getCart(req); if (!cart?.items.some(i => i.id === String(req.params.id)))
    throw new AppError(404, 'Bag item not found.'); await prisma.cartItem.update({ where: { id: String(req.params.id) }, data: { quantity } }); res.json({ success: true, data: await getCart(req) }); }));
