...
ordersRouter.post('/', asyncHandler(async (req, res) => {
  ...
  if (input.couponCode) { const now = new Date(); coupon = await prisma.coupon.findUnique({ where: { code: input.couponCode.toUpperCase() } }); if (!coupon) throw new AppError(400, 'Invalid coupon.'); if (!coupon.isActive || (coupon.startsAt && coupon.startsAt > now) || (coupon.expiresAt && coupon.expiresAt < now) || (coupon.usageLimit != null && coupon.usageCount >= coupon.usageLimit)) throw new AppError(400, 'Expired coupon.'); if (coupon.minimumSpend && subtotal < Number(coupon.minimumSpend)) throw new AppError(400, `This coupon requires a minimum spend of KSh ${Number(coupon.minimumSpend).toLocaleString()}.`) }
