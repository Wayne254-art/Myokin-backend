...
const publicUser = { id: true, firstName: true, lastName: true, email: true, phone: true, role: true, avatar: true } as const
...
authRouter.post('/login', asyncHandler(async (req, res) => {
  ...
  if (!existing || !await bcrypt.compare(data.password, existing.passwordHash)) throw new AppError(401, 'Incorrect Credentials.')
  if(existing.status==='SUSPENDED')throw new AppError(403,'Account has been suspended. Please contact support.')
  if(existing.status==='DISABLED')throw new AppError(403,'Account has been disabled. Please contact support.')
...
async function issueSession(id: string, role: string, res: Parameters<typeof authRouter.post>[1] extends never ? never : any) {
