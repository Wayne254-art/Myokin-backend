...
paymentsRouter.get('/paystack/verify/:reference', asyncHandler(async (req, res) => {
  ...
  if (!payment || (req.user && payment.userId !== req.user.id)) throw new AppError(404, 'Payment not found.')
  ...
  if (data.reference !== reference) throw new AppError(400, 'Payment verification reference did not match.')
...
async function confirmPayment(reference: string, gateway: any) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (purchase?.userId && purchase.items.length) await prisma.cartItem.deleteMany({ where: { cart: { userId: purchase.userId }, OR: purchase.items.map(item => ({ productId: item.productId, variantId: item.variantId })) } })
      return
    }
    catch (error: any) {
